...
This project adapts the Cal.com design tokens for a **Point of Sale desktop application** (Electron + React). Key differences:

### Dark Mode (Added)

The Cal.com system is light-only. We extend it with a full dark palette toggled via CSS class on `<html>`:
...
Toggle via Tailwind `darkMode: 'class'` + React context (`useTheme`). Preference persists in `localStorage`.

### POS-Specific Components

- **pos-keypad** — 3x3/4x3 numeric grid, buttons 48x48px
- **product-search** — Debounced search with keyboard-navigable dropdown
- **cart-panel** — Line items with quantity controls and totals
- **payment-modal** — Payment method selection, amount tendered, change calc
- **cash-summary** — Daily register report

### Typography for POS

- Section padding: `24px` (not 96px)
- Product names/prices: `title-md` (18px)
- Receipt numbers: `font-mono`
- All labels: `text-button` (Inter 14px/600)
...
| POS keypad | 48x48px |
| Cart delete | 44x44px |
| Search result row | 44px tall |
| Sidebar item | 48px tall |
...
All colors map to CSS custom properties in `:root` (light) and `.dark` (dark). Tailwind references `var(--color-*)`. Components never use hex directly.
